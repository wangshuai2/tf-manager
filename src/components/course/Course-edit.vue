...
export default {
    ...
    methods: {
        ...
        submitBtn: function() {
            ...
            this.$http.post('/class/updateClass.json', {cid: this.$route.params.id, gid: this.gradeId, className: this.cName}).then(response => {
    ...
    mounted: function() {
        ...
        this.$http.get('/class/findClass.json?cid=' + this.$route.params.id).then(response => {
            const body = response.body;
            // "gid":102,"school_id":1,"create_time":1487040228000,"name":"一班","id":1}
            if(body.code == 200) {
                this.cName = body.detail.name;
                this.schoolId = body.detail.school_id;
                this.getGradeBySchoolId();
                this.gradeId = body.detail.gid;
            }
        }, response => {
            this.snerr = '网络链接错误，请刷新重试'
        });
