...
export default {
    ...
    mounted: function() {
        ...
        this.$http.get('/student/findStudent.json?stuId=' + this.$route.params.id).then(response => {
            const body = response.body;
            if(body.code == 200) {
                const data = body.detail;
                this.sName = data.stuName;
                this.sPhone = data.phone;
                this.sGender = data.gender;
                this.sAge = data.age;
                this.sAddress = data.address;
                this.cId = '';
                const time = new Date(data.entranceTime);
                this.eTime = time.getFullYear().toString() + '-' + (time.getMonth() + 1).toString() + '-' + time.getDate().toString();
            }
        })
    }
}
